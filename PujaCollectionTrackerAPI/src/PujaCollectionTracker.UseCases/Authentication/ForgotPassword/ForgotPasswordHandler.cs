...
{
  public async ValueTask<Result<string?>> Handle(ForgotPasswordCommand command, CancellationToken cancellationToken)
  {
    try
    {
      ...
      if (user is not null)
      {
        ...
        token = _passwordResetService.GenerateResetToken(command.Email, TimeSpan.FromMinutes(60));
        ...
        // 4. Dispatch the HTML email (wrapped in try-catch so SMTP connection issues do not crash the request).
        try
        {
        ...
        {
          _logger.LogWarning(emailEx, "Failed to send reset email to {Email}. Token generated successfully: {Token}", command.Email, token);
        }
      }
